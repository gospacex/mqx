...
func parseFileFromDisk(path string) (*mqx.Config, string, error) {
	...
	if err != nil { return nil, "", fmt.Errorf("mqttx: read config file: %w", err) }

	var all map[string]*mqx.Config
	if err := yaml.Unmarshal(data, &all); err != nil {
		var single mqx.Config
		if err2 := yaml.Unmarshal(data, &single); err2 != nil { return nil, "", fmt.Errorf("mqttx: parse yaml: %w", err) }
		if configKey == "" { configKey = "default" }
	...
	if configKey == "" {
		...
		return nil, "", fmt.Errorf("mqttx: empty config file")
	}

	cfg, ok := all[configKey]
	if !ok { return nil, "", fmt.Errorf("mqttx: config key %q not found", configKey) }
