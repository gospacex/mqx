...
// ParseFile 从 YAML 文件读取配置 (自带内存热快照)
...
func parseFileFromDisk(path string) (*mqx.Config, string, error) {
	...
	if err != nil { return nil, "", fmt.Errorf("redisx: read config file: %w", err) }

	var all map[string]*mqx.Config
	if err := yaml.Unmarshal(data, &all); err != nil {
		var single mqx.Config
		if err2 := yaml.Unmarshal(data, &single); err2 != nil { return nil, "", fmt.Errorf("redisx: parse yaml: %w", err) }
		if configKey == "" { configKey = "default" }
	...
	if !ok { return nil, "", fmt.Errorf("redisx: config key %q not found", configKey) }
