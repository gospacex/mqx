...
func main() {
	log.Println("=== MQX/Natsx E2E 测试启动 ===")

	// 0. 读取 mq.yaml，根据 trace.enabled 决定是否启动 OTel 链路追踪。
	cfg, _, parseErr := natsx.ParseFile("mq.yaml#nats_js_cluster")
	if parseErr != nil {
	...
	log.Println("=== MQX/Natsx E2E 测试圆满结束 ===")
}
