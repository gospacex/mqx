...
func parseFileFromDisk(path string) (*mqx.Config, string, error) {
	...
	if err != nil { return nil, "", fmt.Errorf("nsqx: read config file: %w", err) }

	var all map[string]*mqx.Config
	if err := yaml.Unmarshal(data, &all); err != nil {
		var single mqx.Config
		if err2 := yaml.Unmarshal(data, &single); err2 != nil { return nil, "", fmt.Errorf("nsqx: parse yaml: %w", err) }
		if configKey == "" { configKey = "default" }
	...
	if configKey == "" {
		...
		return nil, "", fmt.Errorf("nsqx: empty config file")
	}

	cfg, ok := all[configKey]
	if !ok { return nil, "", fmt.Errorf("nsqx: config key %q not found", configKey) }
