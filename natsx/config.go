...
func parseFileFromDisk(path string) (*mqx.Config, string, error) {
	...
	if err != nil {
		return nil, "", fmt.Errorf("natsx: read config file: %w", err)
	}
	...
	if err := yaml.Unmarshal(data, &all); err != nil {
		...
		if err2 := yaml.Unmarshal(data, &single); err2 != nil {
			return nil, "", fmt.Errorf("natsx: parse yaml: %w", err)
		}
		if configKey == "" { configKey = "default" }
	...
	if configKey == "" {
		...
		return nil, "", fmt.Errorf("natsx: empty config file")
	}

	cfg, ok := all[configKey]
	if !ok {
		return nil, "", fmt.Errorf("natsx: config key %q not found", configKey)
	}
